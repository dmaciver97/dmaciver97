...
there will always be some degree of inaccuracy in our prediction. The Kullback-Liebler divergence method allows us to compare the divergence 
between two probability, namely Eq.~\eqref{eq:bayes} and a case scenario.
\begin{align}
	D_{KL}(P\parallel p(\hat{\bf n}_\alpha| y_k(\hat{\bf s}), t)) = \sum P(t) \frac{P(t)}{p(\hat{\bf n}_\alpha| y_k(\hat{\bf s}), t)} 
\end{align} 

Where $P(t)$ is the case scenario we are comparing against. In an ideal
case our model would correctly predict the dimer's orientation with 
100\% accuracy:
\begin{align}
	\label{eq:best}
	p_{best} = P(t) =
...
In reality the distribution from Eq.~\eqref{eq:bayes} will assign some 
non-zero probability to every reference orientation. Therefore the 
divergence of Eq.~\eqref{eq:bayes} at each time step is given as:
\begin{align}
	D_{KL}(p_{best}\parallel p(\hat{\bf n}_\alpha| y_k(\hat{\bf s}), t))
	= \ln \left[\frac{1}{p(\hat{\bf n}_\alpha| y_k(\hat{\bf s}), t)}
	\right]
	\label{eq;kullback}
\end{align}

where a larger value of $D_{KL}$ indicates that our model is less confident 
in its prediction of the dimer's orientation. The divergence $D_{KL}$ thus 
illustrates the 'spread' in the estimated dimer orientation probability 
-- a distribution strongly peaked at some value would give us more 
confidence in that value than a near-uniform distribution where the 
scattering measurement could imply a wide range of possible orientations 
-- but it does not directly indicate our estimates actual accuracy. 
In order to reflect the accuracy of each prediction by using a weighting
function such as:
\begin{align}
	\omega_\alpha = \frac{1}{2} (\hat{\bf n}_{best} \cdot \hat{\bf n}_\alpha+1)
\end{align}

Which is just the normalised dot product between our estimate and the
best possible estimate, so for highly inaccurate estimations $D_{KL}$
is far greater than if we narrowly missed the $\hat{\bf n}_{best}$. With
this in mind the divergence for a single measurement is:

\begin{align}
	D_{KL}(p_{best}\parallel p(\hat{\bf n}_\alpha| y_k(\hat{\bf s}), t))
	= \ln \left[\frac{1}{p(\hat{\bf n}_\alpha| y_k(\hat{\bf s}), t)\omega_\alpha}
	\right]
\end{align}

Later we will show how we set a baseline for evaluating the performance 
of \eqref{eq:bayes} across an entire trajectory and how it handles common
errors in a static light scattering setup. 
...
Brownian force due to the surrounding fluid. MSTM provides the 
necessary T-matrix to compute the optical force via \textit{ott}. 
The Brownian force is found by computing the dimer's diffusion 
tensor according to the analytical solutions provided by Nir and 
Acrivos \cite{Nir1973}. We simulated a polystyrene dimer ($n = 
1.59$) in a suspension of water ($n_{med} = 1.33$) over the 
course of $1 \ s$ with a simulation time step of $1 \times 10^{-5} 
\ s$. We placed the dimer 4 microns below the trap focus at an 
angle $30^{\circ}$ from the horizontal, the resulting trajectory 
is shown below in Sec~\ref{sec:motion}. We chose these initial 
parameters because it demonstrates our model's performance in non 
